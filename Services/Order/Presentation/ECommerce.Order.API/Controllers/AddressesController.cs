...
namespace ECommerce.Order.API.Controllers
{
    ...
    public class AddressesController(IMediator _mediator) : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAddresses(CreateAddressCommand command)
        {
        ...
        [HttpPut]
        public async Task<IActionResult> UpdateAddresses(UpdateAddressCommand command)
        {
            await _mediator.Send(command);
            return NoContent();
        }
